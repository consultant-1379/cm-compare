...
public class DpsTestBase {
    ...
    protected void deleteAllConfigBuckets() throws Exception {
    	userTransaction.begin();
    	final Collection<String> configBuckets = dpsFacade.getAllDataBucketNames();
    	...
    	while (configNames.hasNext()) {
    		...
    		final DataBucket bucket = dpsFacade.getDataBucket(bucketName);
    		if (!bucket.isLiveBucket()) {
    			dpsFacade.deleteDataBucket(bucketName, true);
    		}
    	}
    	userTransaction.commit();
    }
