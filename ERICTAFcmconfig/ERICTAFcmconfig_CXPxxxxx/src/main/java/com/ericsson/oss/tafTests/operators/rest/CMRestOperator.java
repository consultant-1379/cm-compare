...
//package com.ericsson.america.integrationtest.operators.rest;

//import cmpojo.services.ScriptEngine;
import com.ericsson.cifwk.taf.GenericOperator;
import com.ericsson.oss.services.cm.scriptengine.ejb.service.ScriptEngineServiceBean;
...
import com.ericsson.oss.tafTests.getters.rest.CMRestToolRestGetter;
//import com.ericsson.america.integrationtest.operators.RestOperatorExampleOperatorActions;
//sample

public class CMRestOperator implements GenericOperator, CMRestOperatorActions,
		RestOperator {
	// public class RestToolExampleRestOperator implements
	// RestOperatorExampleOperatorActions, RestOperator{

	public static ScriptEngineServiceBean sEngine = new ScriptEngineServiceBean();
	private final static Logger logger = Logger
			.getLogger(CMRestToolRestOperator.class);

	private RestTool restTool;

	public String getScriptEnginetoExcecuteCommand(String command) {
		return sEngine.executeCommand(command);
	}

	// This is the sample stuff Beginning

	/*
	 * This class is "operating" on SUT in REST context. To do it it uses
	 * RestTool
	 */
	 ...
	 */
	@Override
	public boolean executeAuthorisedCall(Host restServer, String userName,
			String password) {
		 ...
		 */
		final List<String> calResponseContent = restTool
				.get(CMRestToolRestGetter.getAuthorisedCallUri());
		System.out.println("RestToolExampleRestGetter.getAuthorisedCallUri()  "
				+ CMRestToolRestGetter.getAuthorisedCallUri());
		final List<RestResponseCode> callResponseCodes = restTool
				.getLastResponseCodes();
		printResponseTimes();
		/*
		 * Verification contains 2 steps: verifying if response code is as
		 * expected and verifying if content is as expected
		 */
		return verifyResponseCodes(callResponseCodes)
				&& verifyResponseContent(calResponseContent);
	}
	...
	private boolean verifyResponseCodes(List<RestResponseCode> responseCodes) {
		...
		for (RestResponseCode callResponseCode : responseCodes) {
			 ...
			 */
			result = result
					&& (callResponseCode == CMRestToolRestGetter.SUCCESFULL_CALL_CODE);
		}
		/*
	...
	private boolean verifyResponseContent(List<String> responseContent) {
		...
		for (String callResponseContent : responseContent) {
			...
			result = result
					&& callResponseContent
							.contains(CMRestToolRestGetter.AUTHORISED_CALL_CONTENT);
		}
		/*
		 * Because method is returning boolean, it is good practice to put
		 * actual result on the test report
	...
	private void printResponseTimes() {
		...
		logger.debug("Average response time: "
				+ restTool.getAverageExecutionTime());
	}

	// This is the sample stuff End
